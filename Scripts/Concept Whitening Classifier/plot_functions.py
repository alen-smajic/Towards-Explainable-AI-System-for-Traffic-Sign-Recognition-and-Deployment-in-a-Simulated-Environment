...
from train_GTSRB import AverageMeter, accuracy #train_places import AverageMeter, accuracy
...
def plot_concept_top50(args, val_loader, model, whitened_layers, print_other = False, activation_mode = 'pool_max'):
    ...
    dst = 'C:/Users/Pasca/OneDrive/Dokumente/Master_3.Semester/Master_3.Semester/SYSL/Concept_whitening/plot/' + '_'.join(args.concepts.split(',')) + '/' + args.arch + str(args.depth) + '/'
    ...
    with torch.no_grad():
        for k in range(begin, end):
            ...
            paths = []
            vals = None
            for i, (inp, _, path) in enumerate(val_loader):
                paths += list(path)
...
def saliency_map_concept_cover(args, val_loader, layer, arch='resnet_cw', dataset='isic', num_concepts=7):
    ...
    dst = "C:/Users/Pasca/OneDrive/Dokumente/Master_3.Semester/Master_3.Semester/SYSL/Concept_whitening/plots/" #'/usr/xtmp/zhichen/temp_plots_layer1_3/'
    try:
        os.mkdir(dst)
    except:
        pass
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            for j in range(num_concepts):
                #save_folder = os.path.join(dst, "concept_"+str(j))
                save_folder = dst + "concept_"+str(j)
                ...
                image.save(save_folder + "/" + str(i)+'.png', 'PNG')
                print("saved: " + str(j))
